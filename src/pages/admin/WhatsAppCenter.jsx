...
function Summary({ health, loading, reload }) {
  ...
  const cards = [
    ...
    ['WABA Subscription', health.meta?.subscription?.subscribed ? 'Subscribed' : 'No confirmada', health.meta?.subscription?.accessible ? `${health.meta?.subscription?.appName || 'Geobooker no encontrada'} · ${health.meta?.subscription?.appCount || 0} app(s)` : 'Meta no permitió consultar subscribed_apps.', health.meta?.subscription?.subscribed ? 'good' : 'bad'],
    ['Webhook messages', health.meta?.subscription?.messagesSubscribed ? 'Subscribed' : 'No confirmado', health.meta?.subscription?.webhookConfigurationAccessible ? `${health.meta?.subscription?.webhookActive ? 'Activo' : 'Inactivo'} · callback ${health.meta?.subscription?.callbackMatches ? 'correcto' : 'distinto'}` : 'Meta no permitió leer la configuración de campos.', health.meta?.subscription?.messagesSubscribed && health.meta?.subscription?.webhookActive && health.meta?.subscription?.callbackMatches ? 'good' : 'bad'],

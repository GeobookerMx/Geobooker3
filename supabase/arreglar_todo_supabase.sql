-- Script para arreglar el error de constraint
-- Ejecutar ESTE SCRIPT en lugar del anterior

-- PASO 1: Ver qué status tienen las campañas actuales
SELECT status, COUNT(*) FROM ad_campaigns GROUP BY status;

-- PASO 2: Actualizar los status inválidos a uno válido
UPDATE ad_campaigns 
SET status = 'active' 
WHERE status NOT IN ('active', 'paused', 'completed', 'cancelled', 'pending_review', 'draft', 'rejected');

-- PASO 3: Ahora sí podemos cambiar el constraint
...
-- PASO 4: Arreglar RLS de user_profiles
...
-- VERIFICACIÓN
SELECT 'Constraint arreglado' as status;
SELECT proname FROM pg_proc WHERE proname = 'create_draft_campaign';

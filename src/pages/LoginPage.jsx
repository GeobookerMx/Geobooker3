...
const LoginPage = () => {
    ...
    try {
      ...
      if (msg.includes('Invalid login credentials') || msg.includes('invalid_credentials')) {
        if (['@icloud.com', '@apple.com', '@me.com'].some(d => formData.email.toLowerCase().includes(d))) {
          toast.error('❌ Si usaste "Continuar con Apple", usa ese botón — no contraseña.', { duration: 7000 });
        } else {
          toast.error(t('login.invalidCredentials'));
        }
      } else if (msg.includes('Email not confirmed')) {
        toast.error('Confirma tu correo. Revisa tu bandeja de entrada.', { duration: 8000 });
      } else if (msg.includes('rate') || msg.includes('too many')) {
        toast.error('⏳ Demasiados intentos. Espera unos minutos.', { duration: 6000 });
      } else {
  ...
  // ✅ FIX OAuth iOS: browser in-app + URL scheme → la app nunca se cierra
      ...
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          <Link to="/welcome" className="inline-block mb-4">
            <img src="/images/geobooker-logo.png" alt="Geobooker" className="h-16 w-auto mx-auto" />
          </Link>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <button onClick={() => handleOAuth('google')} disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white border-2 border-gray-300 text-gray-700 py-3 px-4 rounded-lg hover:bg-gray-50 transition duration-300 font-medium disabled:opacity-50">

...
export default function BibliotecaPage() {
  ...
  const copy = {
    ...
    heroText: isEnglish
      ...
      : 'Una guía práctica, estructurada y responsable para quienes quieren abrir, diagnosticar o mejorar un negocio local desde la idea hasta el crecimiento sostenible.',
    downloadPdf: isEnglish ? 'Download professional PDF' : 'Descargar biblioteca en PDF',
    readRoute: isEnglish ? 'Reading routes' : 'Rutas de lectura',
    docs: isEnglish ? 'Base documents' : 'Documentos base',
    docsDesc: isEnglish
      ? 'The first editorial block has professional editing and is available as a branded PDF.'
      : 'El primer bloque editorial cuenta con revisi\u00f3n profesional y est\u00e1 disponible como PDF con identidad Geobooker.',
  ...
  return (
    <div className="min-h-screen bg-[#f6f1e6] text-slate-950">
      <SEO
        title={isEnglish ? 'Geobooker Library 2026 | Business guides' : 'Biblioteca Geobooker 2026 | Guías de negocio'}
        description={copy.heroText}
        image="/images/geobooker-og-image.png"
        url="/biblioteca"
        keywords="biblioteca geobooker, abrir negocio, guía de negocios, emprendimiento, negocio local, marketing local, sostenibilidad, CRM"
        breadcrumbs={[
          { name: 'Geobooker', item: '/' },
          ...
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              <a
                ...
                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-geoPurple px-6 py-4 text-sm font-black uppercase tracking-[0.16em] text-white shadow-xl transition hover:-translate-y-0.5 hover:bg-slate-950"
              >
          ...
          <div className="relative">
            <div className="rounded-[2rem] border border-slate-900/10 bg-slate-950 p-6 text-white shadow-2xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <Pill tone="cyan">Método Geobooker</Pill>
              ...
              <div className="mt-7 grid gap-3">
                {[
                  ['Idea', 'Del impulso inicial a una decisión validada.'],
                  ['Constructor', 'Psicología, paciencia, energía y criterio.'],
                  ['Operación', 'Procesos, finanzas, ventas y seguimiento.'],
                  ['Crecimiento', 'Visibilidad, reputación, Ads, CRM y sostenibilidad.']
                ].map(([title, text]) => (
      ...
      <section className="bg-white py-16">
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          <SectionHeader
            ...
            title={isEnglish ? 'Start where your business is.' : 'Empieza donde está tu negocio.'}
            description={isEnglish
              ? 'The library is organized as a working method, not as random articles. Choose the path that matches your current stage.'
              : 'La Biblioteca está organizada como método de trabajo, no como artículos sueltos. Elige la ruta que corresponde a tu etapa actual.'}
          />
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 md:px-8">
        <SectionHeader
          eyebrow={copy.docs}
          title={isEnglish ? 'A serious first editorial edition.' : 'Una primera edición editorial seria.'}
          ...
          {libraryDocuments.map((doc) => {
            ...
            return (
              <article key={doc.no} className="group rounded-[1.75rem] border border-slate-900/10 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                ...
                >
                  {isEnglish ? 'Open chapter' : 'Ver capítulo'}
            ...
            <div>
              ...
              <p className="mt-5 text-base leading-7 text-slate-300">
                {isEnglish
                  ...
                  : 'Después de aprobar esta base, podemos crear versiones por categoría: restaurantes, talleres, farmacias, belleza, logística, proveedores y servicios profesionales.'}
              </p>
            </div>
            ...
            <div className="grid gap-8 md:grid-cols-[1fr_0.9fr] md:items-center">
              <div>
                <div className="flex flex-wrap gap-2">
                  <Pill tone="emerald">Edición profesional</Pill>
              ...
              <div className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-6">
                ...
                <p className="mt-4 text-sm leading-6 text-slate-600">
                  {isEnglish
                    ? 'The downloadable edition includes professional editing, Geobooker branding, page numbering, purposeful iconography and a print-ready PDF export.'
                    : 'La edición descargable incluye la revisión ortográfica y una maquetación optimizada para lectura e impresión. El contenido continúa sujeto a futuras actualizaciones editoriales.'}
                </p>
                <p className="mt-4 text-sm leading-6 text-slate-600">
                  {isEnglish
                    ? 'This material is informational and does not replace legal, tax, financial, psychological or professional advice.'
                    : 'Este material es informativo y no sustituye asesoría legal, fiscal, financiera, psicológica ni profesional especializada.'}
                </p>
                <div className="mt-5 flex items-center gap-2 text-sm font-black text-slate-700">
                  <Globe2 className="h-4 w-4 text-geoPurple" />
                  {isEnglish ? 'Global editions planned after Spanish review.' : 'Versiones globales previstas después de aprobar la edición en español.'}
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="bg-white px-4 py-10 text-center">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-4 text-sm leading-6 text-slate-500">
            {isEnglish
              ? 'Version 0.2. Professionally edited Spanish-language guides with branded, downloadable PDFs. English editions remain a future editorial phase.'
              : 'Versión 0.2. Biblioteca Geobooker 2026 con revisión profesional y edición descargable en PDF.'}
          </p>
        </div>
      </section>
    </div>
